...
def tanh_activation(Z):
    #z = 1/(1 + np.exp(-Z)) # trying sigmoid instead of tanh
    #return z  
    return np.tanh(Z)
...
def tanh_activation_backward(x,top_diff):
    #output = tanh_activation(x) * (1 - tanh_activation(x))
    output = np.tanh(x)
    return (1.0 - np.square(output)) * top_diff
    #return output * top_diff
...
def Normalise(x): ##Normalise around 0 
    mean = np.mean(x)
    variance = np.var(x)
    if variance != 0.0:
        ...
        #print("variance = 0 ")
        return  x # if no variance -> all values are same -> dont normalise

def Rnn_forward(input, input_weights, activation_weights, prev_memory, hid_bias):
    forward_params = []
    W_frd = np.dot(activation_weights,prev_memory)
    U_frd = np.asarray(np.dot(input_weights,np.transpose(np.asmatrix(input)))) #tansposing to match 8x1 input
...
def full_forward_prop(T, input_seq ,input_weights,activation_weights,prev_memory,output_weights, hid_bias, out_bias):
    ...
    for t in range(0,T):
        ...
        prev_ht_activation = Normalise(curr_activation)
        memory["ht" + str(t)] = prev_ht_activation
        memory["params" + str(t)] = params
    yt_unactivated = np.asarray(np.dot(output_weights,  prev_ht_activation)) + out_bias
...
def sgd_step(learning_rate, dU,dW,dV,db_hid,db_out, input_weights, activation_weights,output_weights, hid_bias,out_bias ):
    ...
    output_weights -=(learning_rate/(T/4)) * dV
    out_bias -=(learning_rate/(T/4)) * db_out
...
if (bypass_training == False):
    for epoch in range(nepoch):
            ...
            for i in range(batch*batch_size, (batch+1)*batch_size):
                ...
                if row_wise_seq == True:
                    ...
                    if predicted_label == label_value:
                        correct_pred_training = correct_pred_training + 1
        ...
        training_loss = loss / training_size
        ...
        print("%s: Loss after  epoch=%d: %f" % (time,epoch, training_loss))
        ...
        """print("gradient values after epoch : {}----------------------------------------------\n".format(epoch))
        print ("\ndw:-----\n")
        print(str(dW))
        print("dU: --------\n")
        print(str(dU))
    ...
    plt.plot([ x for x in range(nepoch)],losses)
    plt.title("Training Loss")
    #plt.show()
    plt.savefig('training_loss_graph.png', bbox_inches='tight')
...
test_losses=[]
correct_predictions ={}
file = open( "log_bias_no_neg.txt",'w+')
...
file.close()
plt.plot([ x for x in range(test_size)],test_losses)
plt.title("Test Loss")
#plt.show()
plt.savefig('test_loss_graph.png', bbox_inches='tight')
